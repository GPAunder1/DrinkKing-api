...
namespace :cache do
  ...
  namespace :list do
    ...
    task :production => :config do
      ...
      keys = DrinkKing::Cache::Client.new(@api.config).keys
  ...
  namespace :wipe do
    ...
    task :production => :config do
      ...
      if $stdin.gets.chomp.downcase == 'y'
        ...
        wiped = DrinkKing::Cache::Client.new(@api.config).wipe
...
namespace :queues do
  ...
  task :purge => :config do
    ...
    puts "Queue #{@api.config.EXTRACT_QUEUE} purged"
  rescue StandardError => e
...
namespace :worker do
  namespace :run do
    ...
    task :dev => :config do
      sh 'RACK_ENV=development bundle exec shoryuken -r ./workers/recommend_drink_worker.rb -C ./workers/shoryuken_dev.yml'
    end

    desc 'Run the background cloning worker in testing mode'
    task :test => :config do
      sh 'RACK_ENV=test bundle exec shoryuken -r ./workers/recommend_drink_worker.rb -C ./workers/shoryuken_test.yml'
    end

    desc 'Run the background cloning worker in production mode'
    task :production => :config do
      sh 'RACK_ENV=production bundle exec shoryuken -r ./workers/recommend_drink_worker.rb -C ./workers/shoryuken.yml'
    end
  end
end
